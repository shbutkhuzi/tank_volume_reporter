...
void EEPROM_Write (uint16_t page, uint16_t offset, uint8_t *data, uint16_t size)
{
	...
	for (int i=0; i<numofpages; i++)
	{
		...
		while(HAL_I2C_IsDeviceReady(EEPROM_I2C, EEPROM_ADDR, 100, 100) != HAL_OK);  // 100 trials, 100ms until error is returned
		...
		//HAL_Delay (WRITE_CYCLE_TIME);  // Write cycle delay (5ms)
	}
}
...
void EEPROM_PageErase (uint16_t page)
{
	...
	while(HAL_I2C_IsDeviceReady(EEPROM_I2C, EEPROM_ADDR, 100, 100) != HAL_OK);
	//HAL_Delay (WRITE_CYCLE_TIME);  // write cycle delay
}
